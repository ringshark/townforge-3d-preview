...
var ASM_CONSTS = {
  141042: () => { if (document.fullscreenElement) return 1; },  
 141088: () => { return document.getElementById('canvas').width; },  
 141140: () => { return parseInt(document.getElementById('canvas').style.width); },  
 141208: () => { document.exitFullscreen(); },  
 141235: () => { setTimeout(function() { Module.requestFullscreen(false, false); }, 100); },  
 141308: () => { if (document.fullscreenElement) return 1; },  
 141354: () => { return document.getElementById('canvas').width; },  
 141406: () => { return screen.width; },  
 141431: () => { document.exitFullscreen(); },  
 141458: () => { setTimeout(function() { Module.requestFullscreen(false, true); setTimeout(function() { canvas.style.width="unset"; }, 100); }, 100); },  
 141591: () => { return window.innerWidth; },  
 141617: () => { return window.innerHeight; },  
 141644: () => { if (document.fullscreenElement) return 1; },  
 141690: () => { return document.getElementById('canvas').width; },  
 141742: () => { return parseInt(document.getElementById('canvas').style.width); },  
 141810: () => { if (document.fullscreenElement) return 1; },  
 141856: () => { return document.getElementById('canvas').width; },  
 141908: () => { return screen.width; },  
 141933: () => { return window.innerWidth; },  
 141959: () => { return window.innerHeight; },  
 141986: () => { if (document.fullscreenElement) return 1; },  
 142032: () => { return document.getElementById('canvas').width; },  
 142084: () => { return screen.width; },  
 142109: () => { document.exitFullscreen(); },  
 142136: () => { if (document.fullscreenElement) return 1; },  
 142182: () => { return document.getElementById('canvas').width; },  
 142234: () => { return parseInt(document.getElementById('canvas').style.width); },  
 142302: () => { document.exitFullscreen(); },  
 142329: ($0) => { document.getElementById('canvas').style.opacity = $0; },  
 142387: () => { return screen.width; },  
 142412: () => { return screen.height; },  
 142438: () => { return window.screenX; },  
 142465: () => { return window.screenY; },  
 142492: ($0) => { navigator.clipboard.writeText(UTF8ToString($0)); },  
 142545: ($0) => { document.getElementById("canvas").style.cursor = UTF8ToString($0); },  
 142616: () => { document.getElementById('canvas').style.cursor = 'none'; },  
 142673: ($0, $1, $2, $3) => { try { navigator.getGamepads()[$0].vibrationActuator.playEffect('dual-rumble', { startDelay: 0, duration: $3, weakMagnitude: $1, strongMagnitude: $2 }); } catch (e) { try { navigator.getGamepads()[$0].hapticActuators[0].pulse($2, $3); } catch (e) { } } },  
 142929: ($0) => { document.getElementById('canvas').style.cursor = UTF8ToString($0); },  
 143000: () => { if (document.fullscreenElement) return 1; },  
 143046: () => { return window.innerWidth; },  
 143072: () => { return window.innerHeight; },  
 143099: () => { if (document.pointerLockElement) return 1; },  
 143146: ($0, $1, $2, $3, $4) => { if (typeof window === 'undefined' || (window.AudioContext || window.webkitAudioContext) === undefined) { return 0; } if (typeof(window.miniaudio) === 'undefined') { window.miniaudio = { referenceCount: 0 }; window.miniaudio.device_type = {}; window.miniaudio.device_type.playback = $0; window.miniaudio.device_type.capture = $1; window.miniaudio.device_type.duplex = $2; window.miniaudio.device_state = {}; window.miniaudio.device_state.stopped = $3; window.miniaudio.device_state.started = $4; miniaudio.devices = []; miniaudio.track_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == null) { miniaudio.devices[iDevice] = device; return iDevice; } } miniaudio.devices.push(device); return miniaudio.devices.length - 1; }; miniaudio.untrack_device_by_index = function(deviceIndex) { miniaudio.devices[deviceIndex] = null; while (miniaudio.devices.length > 0) { if (miniaudio.devices[miniaudio.devices.length-1] == null) { miniaudio.devices.pop(); } else { break; } } }; miniaudio.untrack_device = function(device) { for (var iDevice = 0; iDevice < miniaudio.devices.length; ++iDevice) { if (miniaudio.devices[iDevice] == device) { return miniaudio.untrack_device_by_index(iDevice); } } }; miniaudio.get_device_by_index = function(deviceIndex) { return miniaudio.devices[deviceIndex]; }; miniaudio.unlock_event_types = (function(){ return ['touchend', 'click']; })(); miniaudio.unlock = function() { for(var i = 0; i < miniaudio.devices.length; ++i) { var device = miniaudio.devices[i]; if (device != null && device.webaudio != null && device.state === window.miniaudio.device_state.started) { device.webaudio.resume().then(() => { Module._ma_device__on_notification_unlocked(device.pDevice); }, (error) => {console.error("Failed to resume audiocontext", error); }); } } miniaudio.unlock_event_types.map(function(event_type) { document.removeEventListener(event_type, miniaudio.unlock, true); }); }; miniaudio.unlock_event_types.map(function(event_type) { document.addEventListener(event_type, miniaudio.unlock, true); }); } window.miniaudio.referenceCount += 1; return 1; },  
 145304: () => { if (typeof(window.miniaudio) !== 'undefined') { window.miniaudio.referenceCount -= 1; if (window.miniaudio.referenceCount === 0) { delete window.miniaudio; } } },  
 145468: () => { return (navigator.mediaDevices !== undefined && navigator.mediaDevices.getUserMedia !== undefined); },  
 145572: () => { try { var temp = new (window.AudioContext || window.webkitAudioContext)(); var sampleRate = temp.sampleRate; temp.close(); return sampleRate; } catch(e) { return 0; } },  
 ...
 148571: ($0) => { return miniaudio.get_device_by_index($0).webaudio.sampleRate; },  
 148637: ($0) => { var device = miniaudio.get_device_by_index($0); if (device.scriptNode !== undefined) { device.scriptNode.onaudioprocess = function(e) {}; device.scriptNode.disconnect(); device.scriptNode = undefined; } if (device.streamNode !== undefined) { device.streamNode.disconnect(); device.streamNode = undefined; } device.webaudio.close(); device.webaudio = undefined; device.pDevice = undefined; },  
 149030: ($0) => { miniaudio.untrack_device_by_index($0); },  
 149073: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.resume(); device.state = miniaudio.device_state.started; },  
 149198: ($0) => { var device = miniaudio.get_device_by_index($0); device.webaudio.suspend(); device.state = miniaudio.device_state.stopped; }
};
